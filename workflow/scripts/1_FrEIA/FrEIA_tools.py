...
def Binning(SampleDf, args):
    ...
    for c in BinnedDf["chr"].unique():
        binEdge = range(1, ChrLenSubset.get(c), args.binSize)
        # Binning and labeling the bins.
        binlab = list(c + "_" + str(l) for l in range(len(binEdge)-1))
        BinnedDf.loc[BinnedDf["chr"] == c, "bin"
                     ] = pd.cut(BinnedDf[BinnedDf["chr"] == c
                                         ]["leftmost_nc_pos"
                                           ], binEdge, labels=binlab)
